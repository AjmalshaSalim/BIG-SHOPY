...
const loadRegister = async (req, res) => {
    try {
        res.render('register', { user: req.session.use, message: '', postive_message: '' })
    }
    catch (error) {
        console.log(error.message)
    }
...
const verifyOtp = async (req, res) => {
    ...
    try {
        ...
        if (req.body.sendotp == req.body.otp) {
            ...
            if (userData) {
                res.render('login', { user: req.session.user, postive_message: " Registered Successfully,  Now Login To Your Account", message: '' })
            }
            else {
                res.render('register', { user: req.session.user, message: "Registration Failed!!", postive_message: '' })
            }
        } else {

            console.log("otp not match");
            res.render('register', { user: req.session.user, message: "Incorrect OTP , Try Again", postive_message: '' })
        }
        ...
        if (userData) {
            ...
            if (passwordMatch) {
                if (userData.is_verified) {
                    ...
                    res.render('login', { user: req.session.user, message: "You Are Blocked By Admin", postive_message: '' })
                }
            } else {
                res.render('login', { user: req.session.user, message: "Password Is Incorrect, Enter Valid Password",postive_message: '' })
            }

        } else {
            res.render('login', { user: req.session.user, message: "Invalid Email Id,",postive_message: '' })
        }
    } catch (error) {
        console.log(error.message);
    }
...
const loadUserOrders = async (req, res) => {
    try {
        ...
        const addressData = await address.find({ userId: usid })
        const orderData = await order.find({ userId: usid }).sort({ createdAt: -1 }).populate("products.item.productId");
        res.render("myOrders", { user: req.session.user, userAddress: adid, userData: user, address: addressData, order: orderData })
    } catch (error) {
        console.log(error.message);
    }
}
